...
- GuideME cultivation-guide changes must keep root `guidebook/` as the only authored Markdown tree, resolve the hard dependency from Maven rather than the root jar, avoid the 21.1.17 `custom_colors` path, and run the focused validator/tests, Gradle tests/build, bounded `runGuide`, and acceptance-server smoke. Startup is not rendering or interaction evidence; every unobserved language, navigation, search, item-index, component/model, reload, key-remap, handbook, and regression surface remains `not_verified`.
