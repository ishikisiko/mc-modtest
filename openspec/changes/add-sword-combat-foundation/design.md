...
The first owner review found that the initial five first-person curves read as only two clearly different actions. The first revision did not alter server cadence, active windows, damage, step timing, or any payload. Instead, it scaled every camera-space displacement around the neutral item pose by exactly `1.20`, began the visible wind-up in the normalized `0.12-0.16` range, placed the strike keyframe in `0.56-0.60`, and kept a recovery silhouette active through `0.84-0.88` before returning to neutral at `1.00`. This redistributed the same server-owned total duration from near-neutral easing into the visible stroke, so the perceived inter-action gap became shorter while each main motion took slightly longer. That fixed factor is retained only as implementation history: uniform amplification was not proof that all five silhouettes used the viewport effectively.

The dedicated item-transform hook has no render buffer or player-arm renderer and therefore cannot draw a skin hand by itself. Re-enabling PAL's rejected full-body first-person path would reintroduce the proven scale/clipping defect, so the hand is a separate client render layer instead. `ClientCombatBootstrap` explicitly registers one `RenderHandEvent` listener that renders before the ordinary Qingfeng item pass, does not cancel the event, and shares the animator's corrected move frame or neutral fallback rather than running an independent clock. `QingfengFirstPersonArmModel` is a MyVillage-owned viewmodel hierarchy: a non-rendering `upper_arm` shoulder driver owns a visible `forearm`, that owns a visible `hand`, and an independent skin/sleeve `connector` reaches from below the screen to the computed elbow. The renderer creates wide/slim and right/left variants from the current player skin, honors sleeve visibility, resets every joint each frame, and omits internal segment caps. It does not borrow PAL-mutated or shared `PlayerRenderer` state.

Each of the five moves authors separate shoulder, elbow, and wrist rotations. Forward kinematics derives the distal hand endpoint, and a three-dimensional correction pins that endpoint to the sword grip for either hand. The corrected and scaled elbow becomes the screen-edge connector target, so the chain appears to continue outside the viewport rather than exposing a detached complete arm. The first rejected predecessor merely reduced complete-arm rotations around a different origin and separated hand from handle. The pivot-locked successor fixed that contact but still exposed the complete arm as a floating middle-screen cuboid. The segmented design replaces both. Epic Fight and GeckoLib are architecture references only: no third-party code, model, texture, animation, or dependency is copied or added. The neutral layer remains visible while a visible local player holds Qingfeng in cultivation mode and is absent for off-hand, invisible, non-Qingfeng, or non-cultivation states. Completion returns item and chain to that joined neutral hold; mode/item/lifecycle interruption removes the layer through the same eligibility state. This changes neither PAL playback nor networking and still requires physical-client grip, joint silhouette, connector proportion, occlusion, clipping, and duplicate-render review.

The latest owner review reported no obvious independent-arm problem for now but rejected the still-current action framing because the motion remained concentrated in the lower-right corner. The active calibration target therefore replaces the fixed-factor contract with a projected viewport envelope. In a `960x540`, `16:9`, FOV-70 reference capture, the temporal union of the visible sword-and-arm silhouette from wind-up through late recovery for each move must span at least `0.50` of the viewport on at least one screen axis, intersect the central horizontal band `x=[0.35,0.65]`, and not remain wholly inside the lower-right quadrant. This envelope is the path accumulated across the action, not a demand that one frame occlude half the screen. Per-move camera-space translation, rotation, and scale may be calibrated independently, but the existing normalized wind-up, strike, and recovery ranges, every server-owned `totalTicks` and active window, damage, step timing, and payload remain unchanged. The shared full parent frame and calibrated wrist pivot remain mandatory, and the expanded poses must introduce no near-plane clipping, grip separation, duplicate arm, or camera rotation. The owner's provisional arm observation does not by itself promote the complete five-move arm ledger to `pass`.

The expanded path also exposes a viewmodel-specific conflict: even segmented
geometry can approach the near plane during the broadest item transforms. The
implemented correction is active only during a move. It smoothly scales the
chain uniformly around the distal grip from `1.00` at the neutral endpoint to
`0.45` by normalized progress `0.12`, holds the compact section through the
middle, and restores `1.00` at completion. Grip correction happens before that
scale and the corrected elbow target uses the same value, so the hand remains
on the handle and the connector still terminates at the elbow. Side-only
segment faces additionally prevent internal caps from becoming near-plane
slabs. This correction does not change the third-person player model, neutral
hold, camera, action clock, or server state, and remains subject to owner review
because avoiding clipping does not prove natural anatomy or connector shape.
