...
The existing ritual/altar and gate motifs (e.g. `incense_altar`, `spirit_array`, and a sect gate / 牌坊) SHALL resolve their focal props through the `RITUAL_ANCHOR` and `MARKET_FITTINGS` / signage slots so they place mod blocks under the `full` profile.
