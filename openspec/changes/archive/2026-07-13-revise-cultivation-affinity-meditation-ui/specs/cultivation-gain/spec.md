...
**Reason**: Progress and post-cap stability now use exact ten-eligible-tick
outputs rather than cultivation-year rates; mastery alone retains its
year-based rate.

**Migration**: Normal progress becomes current affinity per ten eligible ticks,
spirit progress becomes 50 per ten eligible ticks, and lifespan remains on its
independent ordinary clock.

### Requirement: Settlement uses 100-tick batches and transient fixed-point remainders
**Reason**: The requested progress cadence is ten eligible ticks and no longer
needs fractional progress remainders.

**Migration**: Settle progress and eligible stability every ten active ticks;
retain one fixed-point remainder only for unchanged year-based mastery.

### Requirement: Reserve funds only applied spirit bonus progress
**Reason**: Spirit meditation now charges a complete layer-priced item batch
directly and grants a fixed total result, so reserve funding is no longer part
of gameplay.

**Migration**: Preserve the stored reserve value in profile v3 but never credit,
spend, scan, convert, or display it. Existing reserve grants no refund or gain.
