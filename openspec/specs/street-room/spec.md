...
This spec captures the meso layer that makes streets read as active, furnished spaces.

## Requirements

### Requirement: Streets present continuous active frontage
Parcel edges that face a street SHALL present an active frontage rather than a blank wall, and along a main street no blank-wall run SHALL exceed the configured threshold.

#### Scenario: A main street is lined with active frontage
- **WHEN** the street-room layer processes a main street
- **THEN** each street-facing parcel edge SHALL present a shopfront, opening, counter, or equivalent active frontage
...
The main street SHALL be wider than lanes and use a higher-grade paving material.
...
Streets and squares SHALL receive furniture scaled to rank, while keeping traversable paths clear. Street-room furniture SHALL be anchored to street or square ground cells, not parcel or template roof cells.
