...
`/myvillage gallery` and `/myvillage gallery original` output as a
distinct column. No civic-specific command SHALL be required to inspect
civic structures.
...
- **THEN** tavern and lord manor structures SHALL appear in a civic
  ...
  courtyard columns.
- **WHEN** a reviewer runs `/myvillage gallery original`
- **THEN** tavern and lord manor structures SHALL still appear in the
  non-cultivation gallery.
