...
This spec defines the two-axis surface model for courtyard and mansion paths:
materials are selected by surface zone, while route shape is selected by path
role. It covers the six-zone vocabulary used by `chinese_mansion` and the
subset realized by smaller courtyard families.

See also (narrative): [docs/ai-kb/16_path_surface_zoning.md](../../../docs/ai-kb/16_path_surface_zoning.md).
...
A formal-route cell and a tour-route cell sitting in the same zone SHALL
resolve to the same material. The routes are distinguished by the ordered
sequence of zones they pass through, not by per-route material.
...
interior (cells not in any of those sets) SHALL remain `open_sky` and resolve
to `GROUND_YARD_OPEN` (grass). The 天井 zone SHALL NOT cover the whole yard.
...
tree, and any segment that would otherwise cut straight through a rockery or
pond SHALL be forced around it by an obstacle set. The tour route SHALL produce
a visible bend at each waypoint.
...
- **THEN** the route SHALL be the concatenation of the W1->W2 and W2->W3
  shortest paths
...
月洞门, so no cell belongs to both the formal backbone and the tour waypoint set.
The passage SHALL apply the existing `moon_gate` motif to the surrounding wall
cells so the opening reads as a 圆洞门.
...
stepping-stone path (`myvillage:rockery_block` cells) SHALL NOT be restored.
The slab bridge SHALL span the pond's narrowest water crossing between the
...
- **AND** no lily pad SHALL occupy the clear-water lane adjacent to the
  waterside bridge or 水边廊 water-edge row.
