...
The `chinese_mansion` family ships a 3-进 大宅 layout as six NBTs
(`chinese_mansion_001..006.nbt`). Generated by
`tools/generate_compound_library.py --group chinese_mansion --count 6`.

**Plan sequence (north gate, south-facing axis):**

```
street gate → 照壁 (off-axis 照壁侧立) → 倒座 side alley → 前院
           → 仪门 (≥3-cell passage) → 主院 (敞厅 open_hall + 正房 on plinth)
...
- `open_hall`: 敞厅; front facade resolves through `FACADE_OPEN` (columns, no
  full front wall); placed on main_yard plinth
- `side_wing`: 厢房; same as 一进
...
**Validation:** `validate_mansion` (not `validate_compound`). Checks: perimeter,
2 inner gates, ≥3-cell passages, `open_hall` slot, garden parcels, ground holes,
voxel walkability from gate through all yards to 花园 endpoints. Report at
`reports/chinese_mansion_compound_library_report.json`.
...
See also: [chinese-mansion-compound](../../openspec/changes/rebuild-jiangnan-mansion/specs/chinese-mansion-compound/),
